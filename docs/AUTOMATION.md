...
기준일 2026-07-28. 아래에는 **현재 바로 실행할 수 있는 것**과 **추가 구현이 필요한
설계**가 함께 나온다. 아직 자동 운영용 스크립트, 컨테이너, Hermes skill,
GitHub Actions workflow는 저장소에 없다.
...
**권장 시작점은 Hermes가 초안을 만들고 사람이 승인하는 방식이다.** 완전 자동 발행은
최소 1주간 dry-run 결과와 실패율을 확인한 뒤 별도로 켠다.
