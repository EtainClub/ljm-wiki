...
> 서비스 이름은 `src/lib/brand.ts` 가 정본이다. 처음에는 '같은 사건, 다른 제목' 이
> 이름이었는데, 방법을 설명할 뿐 주제를 말하지 않아 첫 화면에서 무엇에 대한
> 사이트인지 알 수 없었다. 지금은 그 문구를 부제로 쓴다.

> 하나의 사건에 대해 언론·유튜브 채널이 **어떤 제목을 달았는지**, 그리고 **어디가 다루지 않았는지**를 하루 한 페이지로 보여준다.

이 문서는 **왜 이렇게 만들었는가**를 적는다.
무엇이 어디에 있고 언제 움직이는지는 [ARCHITECTURE.md](ARCHITECTURE.md),
